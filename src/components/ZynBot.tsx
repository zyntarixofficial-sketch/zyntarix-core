...
export function ZynBot({ userCredits = 0, onTransferToPromptToNexa }: ZynBotProps) {
          ...
          <div className="p-4 flex-1 overflow-y-auto space-y-3 bg-slate-50/50">
            {messages.map((m, idx) => (
              ...
              >
                ...
                {/* Transfer to Nexa Builder Button if prompt is provided by Zyn */}
                {m.sender === "zyn" && onTransferToPromptToNexa && idx > 0 && (
                  <button
                    onClick={() => onTransferToPromptToNexa(m.text)}
