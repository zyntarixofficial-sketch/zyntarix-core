...
export const ZynBot: React.FC<ZynBotProps> = ({ onTransferPromptToNexa }) => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const zynReply = data.reply || "Ami apnar system architecture design korte ready. Ki prokarer app build korte chan?";
      ...
      setMessages((prev) => [
        ...
        {
          sender: "zyn",
          text: "Offline thinking: Shondor idea! Eita Nexe cluster-e pathiye pura code generate kora jabe.",
        },
      ]);
    } finally {
      setIsTyping(false);
    ...
    <div className="fixed bottom-4 right-4 z-50">
      {!isOpen ? (
        <button
          ...
          className="w-11 h-11 sm:w-12 sm:h-12 rounded-full bg-slate-900 text-white flex items-center justify-center shadow-xl border border-slate-700 hover:scale-105 active:scale-95 transition-transform"
        >
          <span className="font-bold text-xs sm:text-sm tracking-wider">ZYN</span>
        </button>
      ) : (
        <div className="w-[calc(100vw-32px)] sm:w-96 max-w-sm h-[440px] sm:h-[460px] bg-white border border-slate-200 rounded-2xl shadow-2xl flex flex-col overflow-hidden">
          ...
          <div className="p-3 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <span className="font-semibold text-xs text-slate-800">Zyn Co-Pilot (Gemini Flash)</span>
            </div>
          ...
          <div className="flex-1 p-3 overflow-y-auto space-y-3 text-xs">
            {messages.length === 0 ? (
              <div className="text-center mt-8 space-y-2 px-2">
                <p className="font-bold text-slate-700">Hey Creator! Ami Zyn ⚡</p>
                <p className="text-slate-400">
                  Apnar app idea, business logic ba feature brainstorm korun. Ready hole 'Transfer to Nexa' diye build shuru korben!
                </p>
              </div>
            ) : (
              messages.map((m, i) => (
                ...
                >
                  ...
                  {m.sender === "zyn" && (
                    <button
                      ...
                      className="mt-2.5 text-[11px] bg-emerald-600 hover:bg-emerald-700 text-white px-2.5 py-1.5 rounded-lg font-bold block w-full text-center shadow-xs transition-colors"
                    >
          ...
          <div className="p-2.5 border-t border-slate-200 bg-slate-50 space-y-2">
            ...
            <div className="flex items-center space-x-2">
              ...
              <input
                ...
                placeholder="Chat with Zyn or refine your app..."
              ...
              <button
                ...
                className="bg-slate-900 disabled:bg-slate-300 text-white px-3 py-1.5 rounded-lg text-xs font-semibold hover:opacity-90 active:scale-95 transition-transform"
              >
                Send
              </button>
            </div>
