    ...
    <>
      <header className="w-full bg-white/95 backdrop-blur-md border-b border-slate-200 px-3 sm:px-6 py-2.5 flex items-center justify-between sticky top-0 z-40">
        ...
        <div className="flex items-center space-x-2 sm:space-x-3">
          <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-indigo-600 flex items-center justify-center font-black text-white text-sm sm:base shadow-sm shrink-0">
            Z
          </div>
        ...
        <div className="flex items-center space-x-2 sm:space-x-3">
          {currentUser ? (
            <>
              ...
              <button
                onClick={() => setMenuOpen(true)}
                className="w-8 h-8 sm:w-9 sm:h-9 rounded-full overflow-hidden bg-gradient-to-tr from-indigo-600 to-violet-500 text-white font-bold text-xs flex items-center justify-center shadow-xs hover:opacity-90 transition-opacity shrink-0"
              >
                {photoURL ? (
            ...
            <button
              ...
              disabled={loading}
              className="bg-indigo-600 hover:bg-indigo-700 text-white px-3 sm:px-4 py-1.5 rounded-xl text-xs font-bold transition-all shadow-xs flex items-center space-x-1.5 cursor-pointer"
            >
              <span>Sign In</span>
            </button>
          )}
            ...
            <div className="space-y-5">
              ...
              <div className="flex items-center justify-between pb-3 border-b border-slate-100">
                <div className="flex items-center space-x-3">
                  <div className="w-10 h-10 rounded-xl overflow-hidden bg-gradient-to-tr from-indigo-600 to-violet-500 text-white font-black text-sm flex items-center justify-center shrink-0">
                    {photoURL ? (
                      <img src={photoURL} alt={displayName} className="w-full h-full object-cover" />
                    ) : (
                      displayName.charAt(0).toUpperCase()
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400 truncate max-w-[160px]">{displayEmail}</p>
                  </div>
                </div>
                <button
                  onClick={() => setMenuOpen(false)}
