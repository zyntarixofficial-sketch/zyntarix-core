...
<html>
  ...
  <body>
    ...
    <script>
      window.createDynamicIcon = function(name) {
      ...
      var iconProxy = new Proxy({}, {
        get: function(target, prop) {
          if (typeof prop === "string" && !target[prop]) {
            target[prop] = window.createDynamicIcon(prop);
          }
          return target[prop];
        }
      });

      window.lucideReact = iconProxy;
    ...
    <script type="text/babel" data-presets="react,typescript">
      ...
      const scopeProxy = new Proxy(window, {
        has: () => true,
        get: (target, prop) => {
          if (prop in target) return target[prop];
          if (typeof prop === 'string' && /^[A-Z]/.test(prop)) {
            return window.createDynamicIcon(prop);
          }
          return undefined;
        }
      });
