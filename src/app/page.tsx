...
export default function Home() {
  ...
  const handleStartBuild = async () => {
    ...
    try {
      ...
      if (resData.success && resData.data) {
        ...
        // Auto transition to live preview
        setTimeout(() => setWorkspaceView("preview"), 1200);
  ...
  // Robust In-Browser Interactive Sandbox Compiler
  const sandboxSrcDoc = useMemo(() => {
    ...
    // Strip React TypeScript types and imports for clean browser execution
    let cleanJsx = currentCode
      ...
      .replace(/<[A-Za-z0-9_]+(\s*\|\s*[A-Za-z0-9_]+)*>/g, "") // removes generic types like <number>, <any>, etc.
      .replace(/:\s*[A-Za-z0-9_\[\]]+(\s*\|\s*[A-Za-z0-9_\[\]]+)*/g, (match) => {
        // preserve object key-values and strip strict variable types
        return match.includes("?") || match.includes('"') || match.includes("'") ? match : "";
      });
    ...
    return `
      ...
      <html>
        <head>
          ...
          <style>
            body { margin: 0; padding: 0; background-color: #020617; color: #f8fafc; font-family: ui-sans-serif, system-ui, sans-serif; overflow-x: hidden; }
        ...
        <body>
          ...
          <script type="text/babel">
            ...
            // Universal Dynamic Proxy for any Lucide Icon
            ...
            // Expose common icons globally
            const { 
              BookOpen, Trophy, Flame, User, Play, CheckCircle, Search, Star, 
              ArrowRight, RefreshCw, Award, Home, Settings, Clock, Bell, Heart,
              Shield, Zap, Check, ChevronRight, Menu, X
            } = iconProxy;
            ...
            try {
              ...
              document.getElementById('root').innerHTML = '<div style="color:#f87171;padding:24px;font-family:monospace;background:#020617;min-height:100vh;"><h3>Live Application Renderer Exception</h3><pre style="white-space:pre-wrap;font-size:12px;margin-top:10px;">' + err.message + '</pre></div>';
            }
          </script>
        </body>
      </html>
        ...
        {activeTab === "home" ? (
          ...
          <div className="flex-1 flex flex-col space-y-3.5 h-full">
            ...
            <div className="bg-slate-950/95 text-slate-100 backdrop-blur-2xl border border-slate-800 rounded-2xl sm:rounded-3xl flex flex-col shadow-2xl overflow-hidden flex-1 min-h-[65vh]">
              ...
              {/* LIVE INTERACTIVE APPLICATION WINDOW */}
