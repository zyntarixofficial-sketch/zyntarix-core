...
export default function Home() {
  ...
  const handleStartBuild = async () => {
    ...
    try {
      ...
      if (resData.success && resData.data) {
        ...
        // Auto transition to preview when ready
        setTimeout(() => setWorkspaceView("preview"), 1000);
  ...
  // Clean code string for sandbox runner
  const currentCode = generatedFiles[0]?.content || "";

  // Dynamic Live Preview HTML Generator
  const sandboxSrcDoc = useMemo(() => {
    if (!currentCode) {
      return `<!DOCTYPE html><html><body style="background:#090d16;color:#94a3b8;font-family:sans-serif;display:flex;align-items:center;justify-content:center;height:100vh;margin:0;"><p>Generating live interactive sandbox...</p></body></html>`;
    }

    // Clean React TSX imports and boilerplate to make it browser-executable via Babel
    let cleanJsx = currentCode
    ...
    return `
      ...
      <html>
        <head>
          ...
          <style>
            body { margin: 0; padding: 0; background-color: #020617; font-family: ui-sans-serif, system-ui, sans-serif; }
          </style>
        </head>
        <body>
          <div id="root"></div>
          <script type="text/babel">
            ...
            const IconMock = () => <span>●</span>;
            const BookOpen = IconMock, Trophy = IconMock, Flame = IconMock, User = IconMock;
            const Play = IconMock, CheckCircle = IconMock, Search = IconMock, Star = IconMock;
            const ArrowRight = IconMock;
            ...
            try {
              ...
              document.getElementById('root').innerHTML = '<div style="color:#f87171;padding:24px;font-family:monospace;background:#090d16;height:100vh;"><h3>Live Preview Notice</h3><p>' + err.message + '</p></div>';
            }
          </script>
        </body>
      </html>
      ...
      <main className="flex-1 w-full max-w-5xl mx-auto px-3 sm:px-6 py-4 sm:py-8 flex flex-col">
        {activeTab === "home" ? (
                  ...
                  {projects.map((proj) => (
                    ...
                    >
                      ...
                      <div className="flex justify-between items-center text-xs text-slate-500 font-medium">
                        ...
                        <span className="text-indigo-600 font-bold">Open App →</span>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <div className="flex-1 flex flex-col space-y-3.5 h-full">
            ...
            <div className="bg-slate-950/95 text-slate-100 backdrop-blur-2xl border border-slate-800 rounded-2xl sm:rounded-3xl flex flex-col shadow-2xl overflow-hidden flex-1 min-h-[60vh]">
              {workspaceView === "logs" && (
              ...
              {workspaceView === "preview" && (
                <div className="flex-1 bg-slate-900 flex flex-col h-full min-h-[600px]">
                  <div className="bg-slate-950 border-b border-slate-800 px-3 py-2 flex items-center justify-between text-xs">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-[11px] font-mono text-slate-400 ml-2">Live Application Preview</span>
                    </div>
                  ...
                  {/* Real-time Browser Rendering Window */}
                  <iframe
