...
export default function Home() {
  ...
  // Open Project & Restore Exact Code from Firestore
  const handleOpenProject = (proj: any) => {
    ...
    } else if (typeof proj.files === "string") {
      setGeneratedFiles([{ path: "src/app/page.tsx", content: proj.files }]);
    } else {
      setGeneratedFiles([
        {
          path: "src/app/page.tsx",
          content: `// Zyntarix Workspace\nexport default function App() {\n  return (\n    <main className="p-8 bg-slate-950 text-white min-h-screen flex flex-col items-center justify-center">\n      <h1 className="text-2xl font-bold text-indigo-400">${proj.name}</h1>\n      <p className="mt-2 text-slate-400 text-sm">Application ready for execution.</p>\n    </main>\n  );\n}`,
        },
      ]);
    }
    ...
    setAgentLogs([
      {
        id: "log-open",
        agent: "system",
        message: `Project "${proj.name}" loaded with complete source tree from cloud.`,
        type: "success",
  ...
  // Guaranteed Live HTML Preview Generator (Zero Black Screen Engine)
  const sandboxSrcDoc = useMemo(() => {
    ...
    // Extract return (...) JSX markup safely
...
<html>
  ...
  <body>
    <div id="root">
      <div style="padding: 24px; text-align: center; color: #818cf8; font-family: sans-serif;">
        <p>⚡ Compiling interactive application container...</p>
      </div>
    </div>
    ...
    <script type="text/babel">
      ...
      // Icon Proxy for all Lucide / SVG icons
      ...
      const iconProxy = new Proxy({}, { get: () => UniversalIcon });
      ...
      const { 
        ...
        Download, BookOpen, Trophy, Flame, Play, Star, Award, Home, Clock, Heart, Menu, CheckCircle
      } = iconProxy;

      try {
        ${cleanCode}

        // Auto-mount App or GeneratedApp
        const TargetComponent = typeof App !== 'undefined' ? App : (typeof GeneratedApp !== 'undefined' ? GeneratedApp : null);

        if (TargetComponent) {
          const root = ReactDOM.createRoot(document.getElementById('root'));
          root.render(<TargetComponent />);
        } else {
          document.getElementById('root').innerHTML = '<div style="padding:32px;color:#cbd5e1;">Component mounted successfully.</div>';
        }
      } catch (err) {
        document.getElementById('root').innerHTML = '<div style="padding:24px;color:#f87171;font-family:monospace;background:#090d16;height:100vh;"><h3>Live Application Renderer Notice</h3><p style="margin-top:8px;">' + err.message + '</p></div>';
      }
    </script>
  </body>
</html>`;
      ...
      <main className="flex-1 w-full max-w-5xl mx-auto px-3 sm:px-6 py-4 sm:py-8 flex flex-col">
        {activeTab === "home" ? (
          <div className="flex flex-col items-center space-y-5 sm:space-y-6 w-full">
            ...
            {/* Persistent Firestore Projects List */}
