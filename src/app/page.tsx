...
export default function Home() {
  ...
  // Close 3-dot dropdown if clicked elsewhere
  useEffect(() => {
  ...
  // Safe Project Actions (Delete, Rename, Duplicate)
  const handleDeleteProject = async (e: React.MouseEvent, projId: string) => {
    ...
    try {
      localStorage.setItem("zyntarix_cached_projects", JSON.stringify(filtered));
    } catch (err) {}
  ...
  const handleConfirmRename = async () => {
    ...
    setProjects(updated);
    try {
      localStorage.setItem("zyntarix_cached_projects", JSON.stringify(updated));
    } catch (err) {}
  ...
  const sandboxSrcDoc = useMemo(() => {
    ...
    const iconMatch = currentCode.match(/import\s*\{([\s\S]*?)\}\s*from\s*['"]lucide-react['"]/);
    let extractedIcons: string[] = [];
    if (iconMatch && iconMatch[1]) {
      extractedIcons = iconMatch[1]
        .split(",")
        .map((s) => s.trim())
    ...
    const iconDeclarations = extractedIcons
      .map((name) => `var ${name} = window.LucideIcons['${name}'];`)
      .join("\n");
...
<html>
  ...
  <body>
    ...
    <script type="text/babel" data-presets="react,typescript">
      ...
      var ShieldCheck = ShieldCheck || window.UniversalIcon;
      var Layers = Layers || window.UniversalIcon;
      var PieChart = PieChart || window.UniversalIcon;
      var PlanCircle = PlanCircle || window.UniversalIcon;
      var TrendingUp = TrendingUp || window.UniversalIcon;
      var TrendingDown = TrendingDown || window.UniversalIcon;
      var Wallet = Wallet || window.UniversalIcon;
      var Settings = Settings || window.UniversalIcon;
      var Search = Search || window.UniversalIcon;
      var User = User || window.UniversalIcon;
      var Bell = Bell || window.UniversalIcon;
      var CheckCircle2 = CheckCircle2 || window.UniversalIcon;
      var RefreshCw = RefreshCw || window.UniversalIcon;
      var ArrowUpRight = ArrowUpRight || window.UniversalIcon;
      var ArrowDownLeft = ArrowDownLeft || window.UniversalIcon;
      var ChevronRight = ChevronRight || window.UniversalIcon;
      var DollarSign = DollarSign || window.UniversalIcon;
      var Filter = Filter || window.UniversalIcon;
      var BookOpen = BookOpen || window.UniversalIcon;
      var Clock = Clock || window.UniversalIcon;
      var Brain = Brain || window.UniversalIcon;
      ...
      <main className="flex-1 w-full max-w-5xl mx-auto px-3 sm:px-6 py-4 sm:py-8 flex flex-col">
        {activeTab === "home" ? (
          <div className="flex flex-col items-center space-y-5 sm:space-y-6 w-full">
            ...
            {/* Persistent Cloud Synced Projects with 3-Dot Management Menu */}
                    ...
                    >
                      <div className="flex justify-between items-start">
                        ...
                        <div className="relative">
                          ...
                          {/* Bolt-style Context Menu Dropdown */}
