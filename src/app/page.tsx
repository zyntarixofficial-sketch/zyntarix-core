...
export default function Home() {
  ...
  // Helper to persist projects array to LocalStorage reliably
  ...
  const handleOpenProject = (proj: any) => {
    ...
    if (proj.files && Array.isArray(proj.files) && proj.files.length > 0) {
      ...
      // Check if project has prompt to re-synthesize or graceful state
      setGeneratedFiles([
        {
  ...
  const handleStartBuild = async () => {
    ...
    try {
      ...
      if (resData.success && resData.data) {
        ...
        // ✅ CRITICAL FIX: Ensure 'files' are saved both locally and in Firestore
  ...
  const handlePatchApp = async () => {
    ...
    try {
      ...
      if (resData.success && resData.data && resData.data.files) {
        ...
        // ✅ CRITICAL FIX: Update files in Local Cache & Firestore on patch
        if (activeProject) {
          const patchedProj = {
            ...activeProject,
            files: updatedFiles,
