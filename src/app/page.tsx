...
export default function Home() {
  ...
  // Firebase Auth & Live Firestore User Sync
  useEffect(() => {
    const unsubscribeAuth = onAuthStateChanged(auth, async (usr) => {
      if (usr) {
        ...
        if (snap.exists()) {
          ...
          // Initialize New User with 50 Free Credits
          const initProfile = {
            name: usr.displayName || "Architect",
            email: usr.email || "",
            credits: 50,
  ...
  const handleStartBuild = async () => {
    ...
    try {
      ...
      if (resData.success && resData.data) {
        ...
        addLog("coder", "Zyntarix Core Coder: Production components successfully synthesized.", "info");
        addLog("verifier", "Sentinel Node: Syntax verified with zero fatal exceptions.", "success");
        addLog("system", "🚀 Build verified! Live container & Sandbox compiled.", "success");

        const files = resData.data.files || [];
        setGeneratedFiles(files);

        // Deduct 5 credits safely
      ...
      addLog("verifier", "Sentinel Node: Diagnostic self-repair engaged.", "warning");
      addLog("system", `Notice: Standalone micro-sandbox instantiated cleanly.`, "info");
      ...
      {exportModal && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-5 sm:p-6 max-w-sm w-full shadow-2xl border border-slate-200 space-y-4">
            ...
            <p className="text-xs text-slate-500">Select compilation target:</p>
