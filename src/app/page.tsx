        ...
        {activeTab === "home" ? (
          ...
          <div className="flex flex-col items-center space-y-6 w-full">
            ...
            <div className="text-center space-y-2 pt-2">
              <h1 className="text-2xl sm:text-4xl font-black tracking-tight text-white drop-shadow-[0_2px_10px_rgba(0,0,0,0.3)]">
            ...
            <div className="w-full bg-white/90 backdrop-blur-2xl border border-white/80 rounded-3xl shadow-[0_12px_40px_rgba(0,0,0,0.15)] overflow-hidden">
              {/* Category Segment Tabs */}
              <div className="flex items-center space-x-1.5 border-b border-slate-200/70 p-2.5 bg-white/50 overflow-x-auto text-xs sm:text-sm">
                {categories.map((cat) => (
                  <button
                    key={cat}
              ...
              <div className="p-4 sm:p-5">
                <textarea
                  ...
                  rows={4}
                  className="w-full text-sm sm:text-base bg-transparent outline-none resize-none placeholder:text-slate-400 text-slate-800 font-medium"
                />
              </div>

              {/* Action Bar */}
              <div className="border-t border-slate-200/70 px-4 sm:px-5 py-3 bg-white/60 flex items-center justify-between">
            ...
            <div className="w-full space-y-3 pt-2">
              ...
              {projects.length === 0 ? (
                <div className="bg-white/80 backdrop-blur-xl border border-white/70 rounded-2xl p-7 text-center text-xs sm:text-sm font-medium text-slate-600 shadow-sm">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                  {projects.map((proj) => (
                    <div
                      ...
                      className="bg-white/90 backdrop-blur-xl border border-white/80 hover:border-indigo-400 p-4 rounded-2xl cursor-pointer transition-all shadow-sm hover:shadow-md space-y-2.5"
                    >
          ...
          <div className="flex-1 flex flex-col space-y-4">
            {/* Terminal Card */}
            <div className="bg-white/90 backdrop-blur-2xl border border-white/80 rounded-3xl flex flex-col shadow-lg overflow-hidden min-h-[300px] flex-1">
              <div className="p-3.5 border-b border-slate-200/70 bg-white/60 flex items-center justify-between">
              ...
              <div className="p-3 border-t border-slate-200/70 bg-white/70">
                <div className="flex items-center bg-white border border-slate-200 rounded-2xl p-1.5 shadow-xs focus-within:border-indigo-500 transition-all">
                  ...
                  <input
                    ...
                    className="flex-1 text-xs sm:text-sm px-3 py-1.5 bg-transparent outline-none text-slate-800 placeholder:text-slate-400"
                  />
                  <button
            ...
            {/* Live Gateway Card */}
            <div className="bg-white/90 backdrop-blur-2xl border border-white/80 rounded-2xl p-4 flex items-center justify-between shadow-md">
              <div>
                <h4 className="font-bold text-xs sm:text-sm text-slate-800">Preview Container</h4>
                <p className="text-[11px] text-slate-500">Dedicated micro-sandbox running live</p>
              </div>
              <div className="flex items-center space-x-2">
                <button className="px-3.5 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-xl shadow-xs transition-colors">
                  Open Live
                </button>
                <button className="px-3.5 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-xl shadow-xs transition-colors">
                  Ship App
                </button>
              </div>
            </div>
