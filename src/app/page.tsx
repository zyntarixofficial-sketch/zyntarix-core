...
export default function Home() {
  ...
  // 1. Initial Local Cache Load (Instant recovery on refresh)
  useEffect(() => {
    try {
      const cachedProjects = localStorage.getItem("zyntarix_cached_projects");
      if (cachedProjects) {
  ...
  // 2. Persistent Firestore Auth & Projects Synchronization
  useEffect(() => {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, async (usr) => {
      if (usr) {
        ...
        // Live Real-Time Firestore Projects Listener
    ...
    setAgentLogs([
      {
        id: "log-open",
        agent: "system",
        message: `Project "${proj.name}" loaded from persistent cloud memory.`,
        type: "success",
  ...
  const handleStartBuild = async () => {
    ...
    try {
      ...
      if (resData.success && resData.data) {
        ...
        // Firestore Permanent Storage under user's Google Account
        if (currentUser) {
  ...
  // Guaranteed In-Browser Visual Sandbox (No Black Screens)
  const sandboxSrcDoc = useMemo(() => {
    ...
    // Strip types, interfaces, and imports
      ...
      <html>
        ...
        <body>
          ...
          <script type="text/babel" data-presets="react">
            ...
            // Universal SVG Icon Fallback for any Lucide or Feather Icon
            ...
            // Expose common icon identifiers
            const { 
            ...
            try {
              ...
              document.getElementById('root').innerHTML = '<div style="color:#f87171;padding:24px;font-family:monospace;background:#090d16;height:100vh;"><h3>Live Application Render Note</h3><p>' + err.message + '</p></div>';
            }
          </script>
        </body>
      </html>
      ...
      <main className="flex-1 w-full max-w-5xl mx-auto px-3 sm:px-6 py-4 sm:py-8 flex flex-col">
        {activeTab === "home" ? (
          <div className="flex flex-col items-center space-y-5 sm:space-y-6 w-full">
            ...
            {/* Persistent Projects Grid (Never lost on refresh) */}
